...
int load(Course cs[]) {
    FILE *fp = fopen("Z:/final/courses.csv", "r");
    if (!fp) return 0;
    int n=0; char line[256];
    while (fgets(line,sizeof line,fp))
        if (sscanf(line,"%19[^,],%99[^,],%d,%99[^\n]",
                   cs[n].code,cs[n].name,&cs[n].hours,cs[n].teacher)==4) n++;
    fclose(fp); return n;
}

void save(Course cs[], int n) {
    FILE *fp=fopen("Z:/final/courses.csv","w");
    for(int i=0;i<n;i++) 
...
void add(Course cs[], int *n) {
    Course c; 
    printf("ชื่อวิชา: "); scanf(" %[^\n]",c.name);
    for(int i=0;i<*n;i++) if(!strcmp(cs[i].name,c.name)) 
        return printf("ชื่อซ้ำ\n"),(void)0;

    printf("รหัสวิชา: "); scanf("%s",c.code);
    for(int i=0;i<*n;i++) if(!strcmp(cs[i].code,c.code)) 
        return printf("รหัสซ้ำ\n"),(void)0;

    printf("ชั่วโมง: "); scanf("%d",&c.hours);
    printf("อาจารย์: "); scanf(" %[^\n]",c.teacher); 
    cs[(*n)++]=c; save(cs,*n); printf("เพิ่มแล้ว\n");
}

void search(Course cs[], int n) {
    char key[100]; 
    printf("ค้นหาด้วยชื่อวิชาหรือรหัส: "); scanf(" %[^\n]",key);
    int found=0;
    for(int i=0;i<n;i++) 
...
void update(Course cs[], int n) {
    char code[20]; 
    printf("รหัสวิชาที่จะแก้: "); scanf("%s",code);
    for(int i=0;i<n;i++) 
        if(!strcmp(cs[i].code,code)) {
            printf("แก้ไข %s (%s)\n", cs[i].name, cs[i].code);
            printf("ชั่วโมงใหม่: "); scanf("%d",&cs[i].hours);
            printf("อาจารย์ใหม่: "); scanf(" %[^\n]",cs[i].teacher);
            save(cs,n); printf("อัพเดทแล้ว\n"); 
            return;
        }
    printf("ไม่พบ\n");
}
...
void del(Course cs[], int *n) {
    char code[20]; 
    printf("รหัสวิชาที่ลบ: "); scanf("%s",code);
    for(int i=0;i<*n;i++) 
        if(!strcmp(cs[i].code,code)) {
            printf("ลบ %s (%s)\n", cs[i].name, cs[i].code);
            for(int j=i;j<*n-1;j++) cs[j]=cs[j+1];
            (*n)--; save(cs,*n); printf("ลบแล้ว\n"); 
            return;
        }
    printf("ไม่พบ\n");
}
